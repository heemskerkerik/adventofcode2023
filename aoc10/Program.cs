...
for (int y = 0; y < bitmap.GetLength(1); y++)
{
    ...
    {
        Console.BackgroundColor = bitmap[x, y] ? ConsoleColor.Green : ConsoleColor.Black;
...
void TranslateGrid()
{
    ...
    for (int x = 0; x < rawGrid[0].Length; x++)
        grid[x, y] = rawGrid[y][x];
}

(int x, int y) FindStartingPoint()
{
...
void ReplaceStartingPosition()
{
    char[] adjacentTiles =
    [
        startingPoint.y > 0
            ? grid[startingPoint.x, startingPoint.y - 1]
            : '.',
        startingPoint.x < grid.GetLength(0) - 1
            ? grid[startingPoint.x + 1, startingPoint.y]
            : '.',
        startingPoint.y < grid.GetLength(1) - 1
            ? grid[startingPoint.x, startingPoint.y + 1]
            : '.',
        startingPoint.x > 0
            ? grid[startingPoint.x - 1, startingPoint.y]
    ...
    grid[startingPoint.x, startingPoint.y] =
        adjacentTiles switch
        {
            ['|' or 'F' or '7', '-' or '7' or 'J', _, _] => 'L',
            ['|' or 'F' or '7', _, _, '-' or 'L' or 'F'] => 'J',
            [_, '-' or '7' or 'J', '|' or 'L' or 'J', _] => 'F',
            [_, _, '|' or 'L' or 'J', '-' or 'L' or 'F'] => '7',
            ['|' or 'F' or '7', _, '|' or 'L' or 'J', _] => '|',
            [_, '-' or '7' or 'J', _, '-' or 'L' or 'F'] => '-',
            _ => throw new Exception("Cannot figure out original starting square."),
        };
}

(int x, int y, Direction dir) FindNextStep()
{
    return grid[startingPoint.x, startingPoint.y] switch
           {
              'F' => (startingPoint.x + 1, startingPoint.y, Direction.East),
              '7' => (startingPoint.x, startingPoint.y + 1, Direction.South),
              'J' => (startingPoint.x - 1, startingPoint.y, Direction.West),
              'L' => (startingPoint.x, startingPoint.y - 1, Direction.North),
              '-' => (startingPoint.x + 1, startingPoint.y, Direction.East),
              '|' => (startingPoint.x, startingPoint.y + 1, Direction.South),
              _ => throw new Exception("Cannot determine next step"),
           };
}
